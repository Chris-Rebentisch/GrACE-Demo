...
async def _probe_embeddings_backend() -> None:
    """Best-effort startup probe for the embeddings backend.

    Cloud embeddings (``GRACE_EMBED_PROVIDER=openai``) skip the Ollama
    ``/api/tags`` check — Demo default is cloud so a student laptop
    ...
    try:
        ...
        try:
            logger.error(
                ...
                error=str(exc),
                advice=(
                    "Ollama is unreachable. If you intended cloud embeddings, "
                    "set GRACE_EMBED_PROVIDER=openai. Local embeddings need "
...
async def lifespan(app: FastAPI):
    ...
    # and probe embeddings (Ollama only when GRACE_EMBED_PROVIDER is not
    # cloud). Both are best-effort and never block or crash startup.
...
def api_health() -> dict:
    """Liveness for GrACE-Demo smoke tests and first-boot checks."""
    return {"status": "ok", "product": "GrACE-Demo"}
